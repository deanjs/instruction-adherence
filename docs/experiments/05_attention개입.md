...
- 단일 층 K/V 이식은 **downstream 전파**돼, 입력에 가까운 초기 층을 건드리면 사실상 코드를
  통째로 교체한 셈이라 자명히 full 회복이 된다(1차 파일럿: `p1a_L0_allG ≈ p1a_full`).
  → **전 층 귀무는 초기 층 전파로 오염**된다.
- 그래서 주 귀무는 **전파 깊이가 비슷한 후기 층(≥ n_layers//2)끼리** 비교한다. 거기서 L25가
  이웃을 압도하는지 본다(참고로 2단계 재현 aux L31/L34도 상위로 나온다).
- **주의(순위 검정의 바닥):** 층 귀무 p는 최소 1/(귀무 층수+1)이라(후기 ~18층이면 ≈0.056)
  아무리 완벽해도 0.05를 못 넘는다. 따라서 층 귀무는 **"L25가 후기 층 중 1등"이라는 순위
  근거**로 쓰고, 유의성 판정은 위의 **부트 CI(0 배제)·대조·용량-반응**에 둔다.
